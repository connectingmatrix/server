...
- Path: `packages/apps/general/src`
...
- Functions (all):
  - `registerRuntimeErrorHandlers` (L26-L26, arrow)
- Exports:
  - None
- Key snippets and use-case mapping:
  - `L26-L26`: Implements `registerRuntimeErrorHandlers` for this module use case.
### `shutdown.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `registerShutdown` (L7-L7, arrow)
  - `shutdown` (L8-L8, arrow)
- Exports:
  - `registerShutdown` (L7)
- Key snippets and use-case mapping:
  - `L7-L7`: Implements `registerShutdown` for this module use case.
  - `L8-L8`: Implements `shutdown` for this module use case.
