...
- Resolvers normalize GraphQL input and auth, then call service functions.
- Scope-bound user/org operations must delegate via `User(context).load()` or `Organisations(context).load({ organizationId })`.
- Do not add fallback/autofill logic in resolvers for tree/workflow/credential/permission/node/org domains.
- GraphQL is the validation and permission source of truth for MCP, Chat, Workflow, and UI adapters.
- Tree shared-placement link/unlink resolvers must preserve owned `CONTAINS_*` hierarchy and use canonical `LINKS`.
- `aiLinkSubjectToCategory` is legacy-named attach behavior and must create `CONTAINS_SUBJECT`.
- Channel, category, subject, and post mutations must delegate through the tree entity/service path rather than MCP handlers.
